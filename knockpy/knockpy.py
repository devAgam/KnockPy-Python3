from modules.zonetransfer import zonetransfer
from modules.header import header
from modules.resolve import resolve
from modules.wildcard import test_wildcard
from modules.save_report import export
from modules.virustotal_subdomains import get_subdomains
...
def print_output(data):
    if data['alias']:
        ...
        for alias in data['alias']:
            ...
            try:
                server_type = str(data['http_response']
                                  ['http_headers']['server'])
            except:
                server_type = ''

            row = (ip_alias+'\t' + str(data['status'])+'\t'+'alias' +
                   '\t' + str(alias)+get_tab(alias)+str(server_type))
            print(row)
            subdomain_csv_list.append(
                ip_alias+','+str(data['status'])+','+'alias'+','+str(alias)+','+escape_csv(str(server_type)))
        ...
        for ip in data['ipaddress']:
            ...
            row = ip+'\t'+str(data['status'])+'\t'+'host'+'\t'+str(
                data['hostname'])+get_tab(data['hostname'])+str(server_type)
            print(row)
            subdomain_csv_list.append(ip+','+str(data['status'])+','+'host'+','+str(
                data['hostname'])+','+escape_csv(str(server_type)))
    else:
...
def main():
        ...
        with open(config_file) as data_file:
            ...
            try:
                ...
                if apikey_vt != '':
                    ...
                    if virustotal_list:
                        init('YES', True)
                        print((json.dumps(virustotal_list,
                                          indent=4, separators=(',', ': '))))
    ...
    wildcard_json = json.loads(test_wildcard(target))
    if wildcard_json['enabled']:
        init('YES', True)
        print((json.dumps(wildcard_json['detected'],
                          indent=4, separators=(',', ': '))))
    else:
        init('NO', True)
    ...
    zonetransfer_json = json.loads(zonetransfer(target))
    if zonetransfer_json['enabled']:
        init('YES', True)
        print((json.dumps(
            zonetransfer_json['list'], indent=4, separators=(',', ': '))))
    ...
    response_resolve = json.loads(resolve(target))
    response_resolve.update(
        {'wildcard': wildcard_json, 'zonetransfer': zonetransfer_json, 'virustotal': virustotal_list})
	...
	'''
    if resolve_host:
        response_resolve = json.dumps(
            response_resolve, indent=4, separators=(',', ': '))
    ...
    for item in subdomain_list:
        ...
        subdomain_resolve = json.loads(resolve(subdomain_target))
        ...
        if subdomain_resolve['hostname']:
            ...
            if wildcard_json['enabled']:
                ...
                if str(status_code) != '' and str(wildcard_code) != '' and str(status_code) == str(wildcard_code):
                    try:
                        content_length = str(
                            subdomain_resolve['http_response']['http_headers']['content-length'])
                    except:
                        content_length = ''
                    try:
    ...
    stats = {'time_start': time_start, 'time_end': time_end,
             ...
             'knockpy': {'version': "devAgam", 'query': sys.argv, 'url': __url__}}
    ...
    if not resolve_host:
        if save_scan_csv:
            exit(export(target, subdomain_csv_list, 'csv'))
        elif save_scan_csvfields:
            exit(export(
                target, subdomain_csv_list, 'csv', save_scan_csvfields))
        elif save_scan_json:
            ...
            report_json = json.dumps(
                report_json, indent=4, separators=(',', ': '))
            exit(export(target, report_json, 'json'))
        else:
            exit()
